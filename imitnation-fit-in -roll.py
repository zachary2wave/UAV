...
random_process = OrnsteinUhlenbeckProcess(size=nb_actions, theta=.15, mu=0., sigma=.3, sigma_min=0)
agent = DDPGAgent(nb_actions=nb_actions, actor=actor, critic=critic, critic_action_input=action_input,
                  memory=memory, nb_steps_warmup_critic=50, nb_steps_warmup_actor=50, random_process=random_process,
                  gamma=0.9, target_model_update=1e-3)
agent.compile(Adam(lr=.0001, clipnorm=1.), metrics=['mae'])

#%%653
history = agent.learning(env, policy, policy_list, nb_steps=1e7, visualize=False, log_interval=1000, verbose=2,
                             nb_max_episode_steps=1000, imitation_leaning_time=1e4, reinforcement_learning_time=3e5)
