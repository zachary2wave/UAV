...
class DDPGAgent(Agent):
    ...
    def __init__(self, nb_actions, actor, critic, critic_action_input, memory,
                 ...
                 warm_up_training_actor=True, warm_up_training_critic=True,
        ...
        # when warm_up training, if training actor  or  critic
        # dr.zee
        self.warm_up_training_actor = warm_up_training_actor
        self.warm_up_training_critic = warm_up_training_critic
    ...
    def backward(self, reward, terminal=False):
        ...
        return metrics
    def warmup_backward(self, reward, terminal=False):
        # Store most recent experience in memory.
        if self.step % self.memory_interval == 0:
            self.memory.append(self.recent_observation, self.recent_action, reward, terminal,
                               training=self.training)
        ...
        if can_train_either and self.step % self.train_interval == 0:
            ...
            if self.step > self.nb_steps_warmup_critic:
                ...
                if len(self.critic.inputs) >= 3:
                    ...
                    state0_batch_with_action = [state0_batch]
                state0_batch_with_action.insert(self.critic_action_input_idx, action_batch)
                "there is a bug  that i can not know what's goning on " \
                "but the bug is about the diminsion of the input "
    ...
    def warm_up(self, observation, action, reward, done_list):
        # Store most warm_data in memory.

        if self.step % self.memory_interval == 0:
            self.memory.append(observation, action, reward, done_list, training=self.training)
        # start wram_up training
        metrics = [np.nan for _ in self.metrics_names]
        # Train the network on a single stochastic batch.
        can_train_either = self.step > self.nb_steps_warmup_critic
        if can_train_either and self.step % self.train_interval == 0:
            experiences = self.memory.sample(self.batch_size)
            assert len(experiences) == self.batch_size
            # Start by extracting the necessary parameters (we use a vectorized implementation).
            ...
            # Update critic, if warm up is over.
            if self.step > self.nb_steps_warmup_critic and self.warm_up_training_critic:
                target_actions = self.target_actor.predict_on_batch(state1_batch)
                assert target_actions.shape == (self.batch_size, self.nb_actions)
                if len(self.critic.inputs) >= 3:
                    state1_batch_with_action = state1_batch[:]
            ...
            # Update actor, if warm up is over.
            if self.step > self.nb_steps_warmup_actor and self.warm_up_training_actor:
                # TODO: implement metrics for actor
                if len(self.actor.inputs) >= 2:
                    inputs = state0_batch[:]
                else:
        ...
        return metrics
    def backward_for_warm_up(self, reward, terminal=False):
        # Store most recent experience in memory.
        if self.step % self.memory_interval == 0:
            self.memory.append(self.recent_observation, self.recent_action, reward, terminal,
                               training=self.training)
        ...
        if can_train_either and self.step % self.train_interval == 0:
            ...
            if self.step > self.nb_steps_warmup_critic:
                ...
                try:
                    ...
                    np.savetxt('error.txt', state0_batch_with_action)
