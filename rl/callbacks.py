    ...
    def on_step_end(self, step, logs={}):
        ...
        for callback in self.callbacks:
            ...
            if callable(getattr(callback, 'on_step_end', None)):
                ...
                callback.on_epoch_end(step, logs=logs)
...
class TrainEpisodeLogger(Callback):
    ...
    def on_episode_end(self, episode, logs):
        ...
        template = '{step: ' + nb_step_digits + 'd}/{nb_steps}: episode: {episode}, episode steps: {episode_steps}, episode reward: {episode_reward:.3f}, {metrics}'+ ',ending:' + str(logs['end'])
        variables = {
            'step': self.step,
